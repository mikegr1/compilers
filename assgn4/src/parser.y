...
var			: ID
			...
			| ID LSQ_BRKT addExpr RSQ_BRKT
			 {
                    tree* varNode = maketree(VAR);
                    varNode->type = ST_lookup_type($1);
                    varNode->sym_type = SCALAR; // Used in checking args vs params. Once indexed, ARRAY is SCALAR.
                    if(varNode->type < 0)
                        yyerror("Undeclared variable");
                    else if(ST_lookup_sym_type($1) != ARRAY)
                        yyerror("Non-array identifier used as an array.");
                    else if($3->type != INT_TYPE)
                        yyerror("Array indexed using non-integer expression.");
                    //If size of array is statically defined (size attr > 0) and it's being indexed by a constant,
                    //check to make sure that the constants is within the array bounds.
                    else if(ST_lookup_size($1) > 0 && $3->nodeKind == INTEGER && $3->val >= ST_lookup_size($1))
...
funcCallExpr    : ID LPAREN argList RPAREN
                 {
                    ...
                    funcCallExprNode->type = ST_lookup_type($1);
                    if(funcCallExprNode->type < 0)
                        yyerror("Undefined function");
                    else if($3->numChildren > ST_lookup_size($1))
                        yyerror("Too many arguments provided in function call.");
                    else if($3->numChildren < ST_lookup_size($1))
                        yyerror("Too few arguments provided in function call.");
                    else if(check_params($3, ST_lookup($1)->params))
                        yyerror("Argument type mismatch in function call.");
                    addChild(funcCallExprNode, maketree(IDENTIFIER));
